...
```{r select variables}
data_summary2 <- data %>% 
  dplyr::select(Treatment, Height, Width, Depth, Roof, Floor, `Logger Depth`) %>%
  mutate(
...
data_summary2 %>%
  ...
  add_p(test = all_categorical() ~ "fisher.test",
      test.args = all_categorical() ~ list(simulate.p.value = TRUE, B = 10000)) #work around for memory error
...
table1 <- 
  ...
  add_p(test = everything() ~ "oneway.test") #this is an one-way ANOVA
...
#post-hoc comparisons among groups are not easily supported currently in gtsummary (technically possible); https://www.danieldsjoberg.com/gtsummary/reference/add_difference_row.tbl_summary.html
```
